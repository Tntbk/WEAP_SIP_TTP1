...
storage_account_key = "WL0dBRwmj1oKVgdvazGskaXE/xtabgZVM3i1JFGonUti/OBlxJJ4AYEV+shyAOvE6wmOvAO4RNqk+AStW+XBoA=="
storage_account_name = "bronzedb"
connection_string = "DefaultEndpointsProtocol=https;AccountName=bronzedb;AccountKey=WL0dBRwmj1oKVgdvazGskaXE/xtabgZVM3i1JFGonUti/OBlxJJ4AYEV+shyAOvE6wmOvAO4RNqk+AStW+XBoA==;EndpointSuffix=core.windows.net"
container_process_name = 'operation'
container_ramans_name = 'raman'
...
def uploadInfo():
    if request.method == 'POST':
       # session['uploaded_data_file_path'] = os.path.join(app.config['UPLOAD_FOLDER'], data_filename)
        ...
        AAA = request.form['Addition_Info_1']
        BBB = request.form['Addition_Info_2']
        CCC = request.form['Addition_Info_3']
        DDD = request.form['Addition_Info_4']
        if AAA == '':
            AAA = 'No'
        if BBB == '':
            BBB = 'No'
        ...
        if DDD == '':
            DDD = 'No'
        print(CusID,ProID,BatchID,Operator,Pilot,AAA,BBB,CCC,DDD)
        ...
        df_ddd = np.full((df_operation.shape[0], 1), DDD)
        ...
        df_operation_full = pd.concat([pd.DataFrame(df_cus_ID, columns=['Cust']), pd.DataFrame(df_pro_ID, columns=['Project_ID']),
                                       pd.DataFrame(df_batch_ID, columns=['BatchID']),
                                        ...
                                        pd.DataFrame(df_ddd, columns=['DDD']),
                                        df_operation], axis=1)
